...
vec3 snapWorldPos(vec2 originalUV, vec4 texelSize, vec3 originalWorldPos) {
    ...
    vec2 dUVdS = dFdxFine(originalUV);
    vec2 dUVdT = dFdyFine(originalUV);

    // 2b.) Invert the fragment from texture matrix
    mat2 dSTdUV = mat2(dUVdT[1], -dUVdT[0], -dUVdS[1], dUVdS[0]) * (1.0 / (dUVdS[0] * dUVdT[1] - dUVdT[0] * dUVdS[1]));
    ...
    vec3 dXYZdS = dFdxFine(originalWorldPos);
    vec3 dXYZdT = dFdyFine(originalWorldPos);
...
void main() {
    ...
    vec4 deltaViewPos = MATRIX_P * MATRIX_V * vec4(deltaPos, 0.0);
    vec2 offset = deltaViewPos.xy * 0.5 + 0.5;
    
    
    o_Albedo = vec4(albedo, ao);
    o_Specular = vec4(offset, metallic, roughness);
