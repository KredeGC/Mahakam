...
namespace Mahakam
{
	...
	void MaterialResourceImporter::OnImport(ryml::NodeRef& node)
	{
		node["Shader"] << m_Material->GetShader().GetID();

		// Material properties
		ryml::NodeRef propertiesNode = node["Properties"];
		propertiesNode |= ryml::MAP;

		const UnorderedMap<std::string, ShaderProperty>& properties = m_Material->GetShader()->GetProperties();
		for (auto& kv : properties)
		{
			switch (kv.second.DataType)
			{
			case ShaderDataType::Float:			propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetFloat(kv.first); break;
			case ShaderDataType::Float2:		propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetFloat2(kv.first); break;
			case ShaderDataType::Float3:		propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetFloat3(kv.first); break;
			case ShaderDataType::Float4:		propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetFloat4(kv.first); break;
			case ShaderDataType::Mat3:			propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetMat3(kv.first); break;
			case ShaderDataType::Mat4:			propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetMat4(kv.first); break;
			case ShaderDataType::Int:			propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetInt(kv.first); break;
			case ShaderDataType::Sampler2D:		propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetTexture(kv.first).GetID(); break;
			case ShaderDataType::SamplerCube:	propertiesNode[ryml::to_csubstr(kv.first)] << m_Material->GetTexture(kv.first).GetID(); break;
			default: break;
			}
		}
	}

	Asset<void> MaterialResourceImporter::CreateAsset(ryml::NodeRef& node)
	{
		Asset<Shader> shader;
		DeserializeYAMLNode(node, "Shader", shader);
	...
	Asset<Texture> MaterialResourceImporter::GetDefaultTexture(const ShaderProperty& property)
	{
	...
	void MaterialResourceImporter::SetupMaterialProperties(const UnorderedMap<std::string, ShaderProperty>& shaderProperties)
	{
