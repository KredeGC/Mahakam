...
namespace Mahakam
{
	GLTFAnimation::GLTFAnimation(const std::filesystem::path& filepath)
	{
		MH_PROFILE_FUNCTION();
	...
	Asset<GLTFAnimation> GLTFAnimation::LoadImpl(const std::filesystem::path& filepath)
	{
		return Asset<GLTFAnimation>(CreateRef<GLTFAnimation>(filepath));
	}
}